...
if [ "$COUNTER" -gt "0" ]
then echo "counter is" $COUNTER >> buffer0.txt 
    sudo python momentum.py $i $holding_period $trigger $market $bin $switch >> buffer0.txt 
...
#   sudo python analyzer.py $i $holding_period $trigger $market $bin $switch
    echo "counter is" $COUNTER >> searchrank0.txt
    sudo python analyzer.py $i $holding_period $trigger $market $bin $switch >> searchrank0.txt 
fi
COUNTER=$((COUNTER+1))
echo $COUNTER
done
